...
// Studio (Home Page > Suburbs Served). Each card links to a real Service
// Area page, giving the internal-linking structure that supports local SEO
// without any automatic list generation.
export async function SuburbsServedSection() {
  const homePage = await getHomePage()
  const section = homePage?.suburbsServed
  const cards = section?.cards

  if (!cards?.length) return null

  return (
    <section id="suburbs-served" className="scroll-mt-16 py-16 sm:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="max-w-2xl">
          <h2 className="font-heading text-3xl font-bold tracking-tight text-foreground text-balance sm:text-4xl">
            {section?.heading ?? 'Suburbs we serve'}
          </h2>
          {section?.subheading && (
